...
const yamlenv = require('../lib/main')
...
t.beforeEach(done => {
  ...
  sinon.stub(yamlenv, 'parse').returns(mockParseResponse)
  done()
})

t.afterEach(done => {
  readFileSyncStub.restore()
  yamlenv.parse.restore()
  done()
})
...
t.test('takes option for path', ct => {
  ...
  const testPath = 'tests/env.yaml'
  yamlenv.config({path: testPath})
...
t.test('takes option for encoding', ct => {
  ...
  yamlenv.config({encoding: testEncoding})
